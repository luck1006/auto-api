...
class Zizhu_BOH_train(interface_boh.interface_boh):
    ...
    def getDetailResources(self):
        ...
        # 获取火车票资源id
        resId1_train = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[0].journeyResources[1].resId')[0])
        resId2_train = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[0].journeyResources[2].resId')[0])
        res1_train_type = str(
        jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[0].journeyResources[1].resType')[0])
        res2_train_type = str(
        jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[0].journeyResources[2].resType')[0])

        #将参数组装成字典
        rsp_dict = dict(journeyId1=journeyId1, journeyId2=journeyId2,resId1=resId1,resType=resType,resSubType=resSubType,resId1_train=resId1_train,resId2_train=resId2_train)
