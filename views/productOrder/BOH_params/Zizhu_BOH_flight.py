...
class Zizhu_BOH_flight(interface_boh.interface_boh):
    ...
    def getDetailResources(self):
        ...
        resType1 = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[1].journeyResources[0].resType')[0])
        resSubType1 = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[1].journeyResources[0].resSubType')[0])
        resId2 = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[1].journeyResources[1].resId')[0])
        resType2 = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[1].journeyResources[1].resType')[0])
        resSubType2 = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[1].journeyResources[1].resSubType')[0])

        # 获取机票资源id
        resId1_flight = str(jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[0].journeyResources[0].resId')[0])
        res1_flight_type = str(
        jsonpath.jsonpath(json.loads(ss['text']), '$.data.rows[0].journeyResources[0].resType')[0])

        #将参数组装成字典
        rsp_dict = dict(journeyId1=journeyId1, journeyId2=journeyId2,resId1=resId1,resId2=resId2,resType1=resType1,resType2=resType2,resSubType1=resSubType1,resId1_flight=resId1_flight,resId1_flight_type=res1_flight_type)
        return rsp_dict                                                                                                      #查询产品详情接口 BOH.NM.ProductController.getDetail



if __name__ == '__main__':

    genp = Zizhu_BOH_flight()
